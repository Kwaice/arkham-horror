...
def downloadDeckArkhamDB(group, x = 0, y = 0):
    deck_url = askString("Insert ArkhamDB deck URL:", "")
    if not deck_url:
        notify("No deck URL provided")
        return
    match = re.search(r'/deck/view/(\d+)', deck_url)
    if not match:
        notify("Invalid deck URL format")
        return
...
def createDeckArkhamDB(deckDict):
    ...
    for card in cards:
        ...
        for _ in range(card['qty']):
            card_obj = deserizlizeCard(cardData)
            card_obj.moveTo(me.deck)
    ...
    try:
        ...
        for card in table:
            ...
            if card.markers[Zero]:
                for _ in range(card.markers[Zero]):
                    createChaosTokenInBag('35137ccc-db2b-4fdd-b0a8-a5d91f453a43')
            if card.Type == "Scenario" and card.Name == "The Vale":
                FHV = True
        ...
        for p in shared.piles:
            ...
            if p == "Special": # Saves Tekeli-li
                for card in shared.piles[p]:
                    if card.Name == "Tekeli-li":
                        if p not in tab['shared']:
                            tab['shared'].update({p: []})
                        tab['shared'][p].append(serializeCard(card))
        ...
        notify("...Chaos Bag saved by the host.")
        if len(tab['shared']) == 2:
            notify("Tekelili deck saved.")
            notify("Each player should save his/her tekeli-li cards.")
    finally:
        clearLock()
...
def loadChaosBag(group, x=0, y=0):
    ...
    try:
        ...
        if len(tab['shared']) == 2:
            notify("Tekeli deck loaded")
            notify("Each player should load his/her tekeli-li cards.")
...
def saveTekelili(group, x=0, y=0):
    mute()
    try:
        tab = {"tekelili": []}
        for card in me.deck:
            if card.Name == "Tekeli-li":
                tab['tekelili'].append(serializeCard(card))
        ...
        with open(filename, 'w+') as f:
            f.write(json().Serialize(tab))
            whisper("Tekeli-li saved to {}".format(filename))
            notify("{} saved his/her tekeli-li cards.".format(me))
    finally:
        clearLock()	
        
def loadTekelili(group, x=0, y=0):
    mute()
    ...
    try:
        ...
        with open(filename, 'r') as f:
            ...
            if tab['tekelili'] is not None and len(tab['tekelili']) > 0:
                deserializePile(tab['tekelili'], me.deck)
                shuffle(me.deck)
                notify("{} loaded his/her tekeli-li cards into his/her deck".format(me))
    finally:
        clearLock()	
...
def saveTable(phase):
    ...
    try:
        ...
        if me._id == 1:
            ...
            for card in table:	
                if card.owner == me or card.owner not in getPlayers() or not isPlayerCard(card):
        ...
        if phase == "": # Manual Save
            notify("{} is saving his game...".format(me))
            filename = saveFileDlg('', '', 'Json Files|*.json')
        else: # Autosave
...
def saveCard(card, x=0, y=0):
    mute()
    notify("{} is saving a card...".format(me))
    try:
        tab = {"table":[]}
        # If Table Host
        if me._id == 1:
        ...
        notify("... card saved by {}".format(me))
        clearLock()

def loadCard(group, x=0, y=0):
    try:
        filename = openFileDlg('', '', 'Json Files|*.json')
        if not filename:
            return
...
def loadTable(phase):
    ...
    try:
        ...
        clearLock()
        if phase:
            notify("Table restored to last {} phase".format(phase))
